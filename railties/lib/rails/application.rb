  ...
  class Application < Engine
    ...
    def message_verifier(verifier_name = 'default')
      @message_verifiers[verifier_name] ||= begin
        if config.respond_to?(:message_verifier_salts)
          salts = config.message_verifier_salts
        else
          salts = {}
        end

        if config.respond_to?(:default_message_verifier_salt)
          default_salt = config.default_message_verifier_salt
        end

        salt = salts[verifier_name] || default_salt || 'application verifier'
